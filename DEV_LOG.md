...
**How to apply**:从 TauricResearch 原版直接 fork,CN 版作为 `alphamind/` 部分(Apache 2.0)的"借鉴学习对象",而不是直接合并代码。
...
**How to apply**:新建 `alphamind/dataflows/a_stock.py`,把 EP11 的 Skill 包装成 vendor;保留原版 `interface.py` 的 `route_to_vendor()` dispatch 模式不动。(最终选择了与 `y_finance.py` 同级的扁平结构,而非嵌套目录。)
...
**How to apply**:在 `alphamind/agents/managers/risk_manager.py` 增加 A 股交易规则约束;Agent 输出建议时必须检验是否落在合法操作空间。
...
- [x] 新建 `alphamind/dataflows/a_stock.py`(~400 行),实现 9 个 vendor 方法:
...
- [x] 改 `alphamind/dataflows/interface.py`:注册 `a_stock` 到 `VENDOR_LIST` + 全部 9 个 `VENDOR_METHODS`
- [x] 改 `alphamind/default_config.py`:四个 `data_vendors` 类别默认切到 `a_stock`
...
- 每次调用自动把当天收盘数据写入 `~/.alphamind/cache/northbound_daily.csv`
...
| 文件 | 改动 | 说明 |
|------|------|------|
| `alphamind/llm_clients/factory.py` | `_OPENAI_COMPATIBLE` 增加 `"minimax"` | 路由 minimax → OpenAIClient |
| `alphamind/llm_clients/openai_client.py` | `_PROVIDER_CONFIG` 增加 `"minimax": ("https://api.minimax.chat/v1", "MINIMAX_API_KEY")` | base_url + API key 环境变量 |
| `alphamind/llm_clients/model_catalog.py` | 新增 `"minimax"` 模型目录(M2.7/M2.7-highspeed/M2.5/M2.5-highspeed) | CLI 选项 + 模型验证 |
...
- **启动方式**：`alphamind-web` CLI 命令 或 `streamlit run web/app.py`
...
| `web/runner.py` | ~110 | 后台线程：创建 AlphaMindGraph、stream 状态、检测 12 阶段完成 |
| `web/progress.py` | ~95 | ProgressTracker：线程安全、12 阶段定义、stage 状态追踪 |
| `web/history.py` | ~55 | 扫描 `~/.alphamind/logs/` 历史 JSON |
| `web/pdf_export.py` | ~170 | fpdf2 PDF 生成：CJK 字体自动检测、封面 + 7 报告 + 辩论 + 风控 |
| `web/launch.py` | ~15 | CLI 启动器：`alphamind-web` 命令入口 |
